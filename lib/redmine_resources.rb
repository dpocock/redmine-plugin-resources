...
module RedmineResources
  ...
  DEFAULT_WORKDAY_LENGTH = 8.0

  def self.default_workday_length
    length = (people_plugin_installed? ? Setting.plugin_redmine_people : settings)['workday_length'].to_f
    length.positive? ? length : DEFAULT_WORKDAY_LENGTH
  end

  def self.workday_length
    length = settings['workday_length'].to_f
    length.positive? ? length : DEFAULT_WORKDAY_LENGTH
  end
